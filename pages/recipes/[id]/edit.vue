<template>
  <div class="max-w-4xl mx-auto space-y-6">
    ...
    <form v-else class="space-y-6" @submit.prevent="onSubmit">
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="space-y-2">
            <div class="flex items-center justify-between">
              ...
              <button v-if="mainPreviewUrl || form.main_image_url" type="button"
            ...
            <div v-if="!(mainPreviewUrl || form.main_image_url)"
            ...
            <div v-else class="rounded-2xl overflow-hidden bg-gray-100 aspect-[4/3] cursor-pointer"
              @click="openMainImagePicker">
        ...
        <div class="space-y-6">
          <div class="space-y-3">
            ...
            <div v-for="(ing, idx) in ingredients" :key="idx" class="grid grid-cols-12 gap-2">
              ...
              <input v-model.number="ing.quantity" type="number" step="0.1"
                class="col-span-3 border rounded-xl px-3 py-2 text-sm" placeholder="К-сть" />
          ...
          <div class="space-y-3">
            ...
            <div v-for="(st, idx) in steps" :key="idx" class="border rounded-2xl p-4 space-y-2">
              ...
              <textarea v-model="st.text" rows="3" class="w-full border rounded-xl px-3 py-2 text-sm"
                placeholder="Опис кроку" />

            <div class="space-y-2">
                <div class="flex items-center justify-between">
                  <label class="text-sm font-medium">Зображення кроку (опційно)</label>
                  <button v-if="st.previewUrl || st.image_url" type="button"
                    class="text-sm border border-gray-300 rounded-full px-3 py-1.5 hover:bg-gray-50"
                ...
                <input :ref="(el) => setStepImageInput(el, idx)" type="file" accept="image/*" class="hidden"
                  @change="(e) => onStepImageChange(idx, e)" />

                <div v-if="!(st.previewUrl || st.image_url)"
                  class="rounded-2xl border border-dashed border-gray-300 bg-gray-50 aspect-[4/3] flex flex-col items-center justify-center gap-2 cursor-pointer hover:bg-gray-100 transition"
      ...
      <button type="submit" class="rounded-full bg-primary text-white px-6 py-2 text-sm hover:bg-orange-500 transition"
        :disabled="saving">
